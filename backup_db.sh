#!/bin/bash
#
# backup_db.sh — Backup automático do PostgreSQL (Docker) + Upload S3
#
# Execução: ./backup_db.sh  (ou via crontab)
# Requer:   Docker rodando com o container "postgres" ativo.
#
# Decisão de Design — pg_dumpall vs pg_dump -Fc:
#   Usamos pg_dumpall porque a instância PostgreSQL hospeda múltiplos bancos de
#   dados de diferentes aplicações. pg_dumpall captura TODOS os databases, roles
#   e permissões em um único dump consistente. Se no futuro for necessário
#   restore granular por banco/tabela, considere adicionar pg_dump -Fc por banco
#   como um passo complementar (não substituto).
# ---------------------------------------------------------------------------
...
BACKUP_FILE="${BACKUP_DIR}/db_backup_all_${TIMESTAMP}.sql.gz"
...
exec 200>"${LOCK_FILE}"
if ! flock -n 200; then
    echo "WARNING: Outra instância do backup já está em execução. Abortando." >&2
    exit 0
fi
...
send_telegram_notification() {
    ...
    if [ "${status}" = "SUCCESS" ]; then
        ...
        message="<b>✅ Backup do PostgreSQL concluído com sucesso!</b>
...
<b>Data:</b> <code>$(date +"%d/%m/%Y %H:%M:%S")</code>"
        ...
        message="<b>❌ Falha no Backup do PostgreSQL!</b>
...
# Credenciais do PostgreSQL lidas do .env (previsível e auditável)
DB_USER="${POSTGRES_USER:-postgres}"
DB_PASS="${POSTGRES_PASSWORD:-}"

if [ -z "${DB_PASS}" ]; then
    echo "ERROR: POSTGRES_PASSWORD não definida no .env ou no ambiente!" >&2
    exit 1
fi

echo "=========================================="
echo "Iniciando Backup do PostgreSQL ($(date))"
echo "=========================================="
...
if ! docker ps --format '{{.Names}}' | grep -q "^postgres$"; then
    echo "ERROR: O container 'postgres' não está em execução!" >&2
    exit 1
fi

# Espera o healthcheck confirmar que o Postgres está pronto (timeout: 30s)
echo "Verificando se o PostgreSQL está pronto..."
for i in $(seq 1 6); do
    if docker exec postgres pg_isready -U "${DB_USER}" -q 2>/dev/null; then
        break
    fi
    if [ "${i}" -eq 6 ]; then
        echo "ERROR: PostgreSQL não ficou pronto após 30 segundos!" >&2
        exit 1
    fi
    sleep 5
done
...
# ---------------------------------------------------------------------------
# 5. Executa pg_dumpall com captura confiável de erros no pipeline
# ---------------------------------------------------------------------------
#
# Problema do pipeline simples (pg_dumpall | gzip):
#   Com pipefail, qualquer falha no pg_dumpall propaga corretamente.
...
FIFO_PATH="${BACKUP_DIR}/.dump_pipe_${TIMESTAMP}"
mkfifo "${FIFO_PATH}"
...
# Executa o pg_dumpall escrevendo no FIFO e captura o exit code
DUMP_EXIT=0
docker exec -e PGPASSWORD="${DB_PASS}" postgres \
    pg_dumpall -U "${DB_USER}" --clean --if-exists > "${FIFO_PATH}" || DUMP_EXIT=$?
...
if [ ${DUMP_EXIT} -ne 0 ]; then
    echo "ERROR: pg_dumpall falhou com código ${DUMP_EXIT}!" >&2
    exit 1
fi
...
DELETED_COUNT=$(find "${BACKUP_DIR}" -type f \( -name "db_backup_all_*.sql.gz" -o -name "db_backup_all_*.sql.gz.sha256" \) -mtime +7 -print -delete | wc -l)
